...
public class SingleNodeHandler implements ResponseHandler, Terminatable,
	...
	public void okResponse(byte[] data, BackendConnection conn) {        
		...
		if (executeResponse) {
			...
			//for_redis
			getRedisParser().ddlFlushToRedis();
	...
	public void fieldEofResponse(byte[] header, List<byte[]> fields,
            ...
            for (int i = 0, len = fields.size(); i < len; ++i)
            {
    			...
    			fieldPkg.read(field);
    			if (redis.isRedis()) {
    				redis.getDbFields().add(new String(fieldPkg.name));
	...
	public void rowResponse(byte[] row, BackendConnection conn) {
		...
		if (redis.isRedis()) {
			...
			for (byte[] v : rowDataPkgRed.fieldValues) {
				redis.getValues().add(new String(v));
			}
		}

		if (redis.isRedis()) {
			for (Object f : redis.getValues()) {
				System.out.println("vvvvv   " + f);
			}
		}
