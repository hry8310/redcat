...
import org.opencloudb.redis.RedisParser;
...
public class MultiNodeQueryHandler extends MultiNodeHandler implements
	...
	public void okResponse(byte[] data, BackendConnection conn) {
		...
		RedisParser redis = getRedisParser();
		redis.ddlFlushToRedis();
		if (executeResponse) {
	...
	public void rowEofResponse(final byte[] eof, BackendConnection conn) {
		...
		getRedisParser().dqlFlushToRedis();
...
//			}
	...
	public void fieldEofResponse(byte[] header, List<byte[]> fields,
		...
		try {
			...
			for (int i = 0, len = fieldCount; i < len; ++i) {
				...
				RedisParser redis = source.getRedisParser();
				redis.addField(new String(fieldPkgTmp.name));
				if (!shouldSkip) {
					field[3] = ++packetId;
	...
	public void rowResponse(final byte[] row, final BackendConnection conn) {
		...
		try {
			...
			if (dataMergeSvr != null) {
				...
				//for_redis
				RowDataPacket rowDataPkgRed = new RowDataPacket(fieldCount);
				rowDataPkgRed.read(row);
				for (byte[] v : rowDataPkgRed.fieldValues) {
					redis.addValue(new String(v));
				}
				row[3] = ++packetId;
	...
	public RedisParser getRedisParser(){
		return   session.getSource().getRedisParser();
	};
