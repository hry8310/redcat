...
public abstract class AbstractRouteStrategy implements RouteStrategy {
	...
	public RouteResultset route(SystemConfig sysConfig, SchemaConfig schema, int sqlType, String origSQL,
		...
		// hry_for_redis
		RedisParser redisParser = new RedisParser(schema);
		redisParser.parse(stmt);
		sc.setRedisParser(redisParser);
		if (redisParser.getNeed() != -1
				&& redisParser.getSqlType() == ServerParse.SELECT) {
			 if (redisParser.getHandler().select()) {
				 return null;
			 }
		}
		...
		if(rrs.getNodes().length==1){
			if (redisParser.getNeed() != -1
					&& redisParser.getSqlType() == ServerParse.UPDATE && redisParser.getAll()!=1 ) {
				redisParser.setRrs(rrs);
				if (redisParser.getHandler().updateToRedis()) {
					return null;
				}
			}
		}
